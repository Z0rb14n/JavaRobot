...
public class JavaRobot /* implements KeyListener, not implemeneted lol*/{
     ...
     * @param y y position to click at
     */
    static void clickAt(int x, int y, byte MouseEvent){
     ...
     * @param a 
     */
    static void simplerType(String a) {
     ...
     */
    static void keyPress(int a_key) {
        bot.keyPress(a_key);
     ...
     */
    static void keyPressCapitalized(int a_key) {
     ...
     */
    static void scrollWheel(int amount) {
     ...
     */
    static void holdType(int hold, int... types){
        bot.keyPress(hold);
     ...
     */
    static void holdType(int hold, char... types){
        bot.keyPress(hold);
     ...
     */
    static void holdType(int hold, String list){
     ...
     */
    static void holdType(int[] hold, String types){
        for(int lol: hold) bot.keyPress(lol);
     ...
     */
    static void simplerType(char character){
     ...
     * @param path Path to save screenshot to
     */
    static void screenShot(Rectangle area, String path) {
        if (path == null || path.length() < 5) throw new IllegalArgumentException("BRUH WHAT");
        String ext4 = path.substring(path.length() - 4, path.length());
        String ext3 = path.substring(path.length()-3, path.length());
        boolean testJPEG = ext4.compareToIgnoreCase("jpeg") == 0;
     ...
     * @param path path to save screenshot to
     */
    static void screenShot(int x, int y, int width, int height, String path) {
        if (path == null || path.length() < 5) throw new IllegalArgumentException("BRUH WHAT");
        String ext4 = path.substring(path.length() - 4, path.length());
        String ext3 = path.substring(path.length()-3, path.length());
        boolean testJPEG = ext4.compareToIgnoreCase("jpeg") == 0;
     ...
     */
    static void screenShot(int x, int y, int width, int height, File path_file) {
